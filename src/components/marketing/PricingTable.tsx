...
const PricingTable: React.FC<PricingTableProps> = ({ plans }) => {
  ...
  useEffect(() => {
    trackClient({ name: "pricing_page_viewed" })
  }, [])
        ...
        {plans.map((plan, index) => (
          ...
          >
            ...
            {plan.isFeatured && (
              ...
              >
                <div className="bg-gradient-to-r from-violet-500 to-indigo-600 text-white px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 shadow-lg">
            ...
            <div className={`
              relative h-full p-8 rounded-xl border-2 transition-all duration-300 backdrop-blur-md
              ${plan.isFeatured
                ? 'border-violet-500/60 bg-gradient-to-br from-violet-50/80 to-indigo-50/80 dark:from-violet-950/30 dark:to-indigo-950/30 dark:border-violet-400/50 shadow-lg shadow-violet-500/10'
                : 'border-border/50 bg-white/70 dark:bg-zinc-950/50'
              }
            `}>
              {/* Plan Header */}
