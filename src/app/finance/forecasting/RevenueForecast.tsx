    ...
    if (!data) {
        return (
            <div className="text-center py-12 text-muted-foreground">
                ...
                <p>Forecast couldn&apos;t load. Refresh the page to try again.</p>
            </div>
        )
    }

    const hasOpportunities = (data.byStage || []).some((s: any) => (s.value ?? 0) > 0)
    if (!hasOpportunities) {
        return (
            <Card className="border-dashed">
                <CardContent className="py-2">
                    <EmptyState
                        ...
                        description="Forecasts use weighted stage probabilities × opportunity value × expected close date. Add a few deals to start seeing projections."
                        action={{ label: "Open pipeline", href: "/pipeline" }}
                        secondaryAction={{ label: "Configure stages", href: "/settings/workspace/pipeline" }}
                    />
                </CardContent>
            </Card>
        )
